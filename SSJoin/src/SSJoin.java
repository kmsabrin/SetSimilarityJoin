...
{   
	...
	public static class MapClass0 extends Mapper<LongWritable, Text, Text, NullWritable>
	{  	 
	...
	public static class Reduce0 extends Reducer<Text, NullWritable, Text, NullWritable>
	{   
	...
	public static class MapClass1 extends Mapper<LongWritable, Text, Text, IntWritable>
	{  	 
   	private final static IntWritable one = new IntWritable(1); // constant counter
	...
	public static class MapClass2 extends Mapper<LongWritable, Text, IntWritable, Text>
	{  	 
	...
	{  	 
   	...
   	{
   		/*
       	// Global frequency sorted token input from Distributed Cache
       	try
       	{
   		...
   		for ( URI cachePath : cacheFiles )
               	{
   	    	System.out.println( cachePath.getPath() );    
       	...
       	*/
       	...
       	try
       	{
           	...
           	for ( int i = 0; i < status.length; ++i )
           	{
               	...
               	if ( fname.charAt(0) != 'p' ) continue; // file name must be part-r-xxxxx
               	BufferedReader d = new BufferedReader(new InputStreamReader(fs.open( status[i].getPath() ) ) );
                           	...
                           	throws IOException, InterruptedException
   	{
       		...
       		Arrays.sort(tokens, new Comparator<String>() {
           		public int compare(String s1, String s2) {
               		...
               		if (hmap.containsKey(s1)) a = hmap.get(s1);
               		if (hmap.containsKey(s2)) b = hmap.get(s2);
               		return a - b;
           		}
       		});
                       	...
                       	throws IOException, InterruptedException
   	{
       	...
       	for ( int i = 0; i < slist.size(); ++i )
       	{
           	...
           	for ( int j = 0; j < slist.size(); ++j )
           	{
               	...
               	if ( Integer.parseInt( v1[0] ) >= Integer.parseInt( v2[0] ) ) continue;
               	double l1 = v1.length;
               	double l2 = v2.length;
               	if ( ( l1 <= l2 ) && ( l2 <= ( 1 / ( simThld * l1 ) ) ) ) continue;
                                                 	 
               	int cap = 0; // cap is for intersection
             	 
               	if ( l1 < l2 )
               	{
                   	HashSet<String> hset = new HashSet<String>(Arrays.asList(v2));
               	...
               	{
                   	HashSet<String> hset = new HashSet<String>(Arrays.asList(v1));
               	...
               	double sim = (double) cap / (double)( v1.length + v2.length - cap - 2 );
           	 
               	//System.out.println( v1[0] + "," + v2[0] + "=" + sim + " for " + key.toString() );
	...
	public int run( String[] args) throws Exception
	{
   	///////////////////////////////////////////////////////////////////////////
   	/*
   	Job job0 = new Job( getConf() );
   	job0.setJarByClass( SSJoin.class );
   	Path in = new Path( args[0] ); // in
   	Path out = new Path( args[1] ); // out0
   	FileInputFormat.addInputPath(job0, in);
   	FileOutputFormat.setOutputPath(job0, out);
   	job0.setMapperClass( MapClass0.class );
   	job0.setReducerClass( Reduce0.class );
   	...
   	*/  	 
   	...
   	//DistributedCache.addCacheFile( new URI("/user/hduser/out2/part-r-00000"), getConf() );
   	//URI [] cacheFiles = DistributedCache.getCacheFiles( getConf() );
   	//if ( cacheFiles != null ) System.out.println( "ADDED " + cacheFiles[0].getPath() );
